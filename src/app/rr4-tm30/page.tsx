...
export default function Rr4Tm30Page() {
  ...
  // clicking RR4/TM30 reveals a small Preview/Download menu.
  ...
  const handleDownload = async (kind: TabKey, rowDate: string) => {
    ...
    try {
      ...
      const filename = filenameFromResponse(res, `${kind.toUpperCase()}_${selectedProperty}_${rowDate}.xlsx`);
  ...
  // Opens the spreadsheet preview in its own tab (/rr4-tm30/preview), rather
  // than as an overlay - the grid gets the whole window, and this page keeps
  // its scroll position and loaded report for when the user comes back.
  const handlePreview = (kind: TabKey, rowDate: string) => {
    if (!selectedProperty) return;
                              ...
                              <div key={kind} className="relative inline-block">
                                ...
                                {openFileMenu === menuKey && (
                                  ...
                                  <div
                                    ...
                                    className="absolute left-0 top-9 flex flex-col w-36 bg-[var(--paper)] border border-[var(--text-primary)] shadow-xl z-[100] p-1"
                                  >
                                    <button
                                      onClick={() => {
