...
// Same 4 columns as the "RR4-TM30-LubdSamui-Gen" reference sheet's own
// TM30-Nationality tab, in the same order (Mews Nationality, Eng, Thai,
// Nationality) - MEWS Nationality is the country NAME (e.g. "Germany"),
// not an ISO code, matching how the original manual spreadsheet's VLOOKUP
// worked; see _resolve_tm30_nationality_codes in sync_service.py for how
// the backend joins a guest's alpha-2 NationalityCode to this table via
// _RR3_COUNTRY_MAP. tm30_code is an ISO 3166-1 alpha-3 code (e.g. "DEU"),
// not numeric like RR4's.
...
export default function Tm30NationalityPage() {
  ...
  const [newForm, setNewForm] = useState({ mews_nationality: "", english_name: "", thai_name: "", tm30_code: "" });
  ...
  const fetchRows = async () => {
    ...
    const { data, error } = await supabase
      .from("tm30_nationality_codes")
      .select("id, mews_nationality, english_name, thai_name, tm30_code")
      .order("mews_nationality");
    if (error) {
      console.error("Failed to fetch tm30_nationality_codes:", error.message);
  ...
  // fire a write. get_tm30_report reads this table fresh on every export,
  // so a save here is live immediately - no separate "publish" step.
  const handleFieldSave = async (row: NationalityRow, field: keyof Omit<NationalityRow, "id">, value: string) => {
    const trimmed = field === "tm30_code" ? value.trim().toUpperCase() : value.trim();
  ...
  const handleAdd = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        tm30_code: newForm.tm30_code.trim().toUpperCase(),
      })
      .select("id, mews_nationality, english_name, thai_name, tm30_code")
      .single();
    if (error) {
      ...
      setNewForm({ mews_nationality: "", english_name: "", thai_name: "", tm30_code: "" });
      setIsAdding(false);
    }
    setAddingBusy(false);
  };
  ...
  const handleDelete = async () => {
    ...
    const { error } = await supabase.from("tm30_nationality_codes").delete().eq("id", deletingRow.id);
    if (error) {
  ...
  return (
    <div className="p-6 bg-white min-h-screen text-slate-900 font-sans relative">
      <PageHeader
        title="TM30-Nationality"
        description="MEWS Nationality -> Immigration TM30 alpha-3 code, used by every TM30 export. Edits apply immediately - no deploy needed."
      />
      ...
      <div className="mb-6">
        ...
        >
          ...
          Read Me - where TM30-Nationality is used in the export
        </button>
        {readmeOpen && (
          <div className="mt-3 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
            <p className="text-sm text-slate-600 mb-3">
              A row&apos;s <span className="font-bold">TM30-Nationality</span> code fills exactly one column in the TM30 export - Column F (สัญชาติ / Nationality):
            </p>
...
{`"nationality": tm30_code,  # F - สัญชาติ`}
            </pre>
            <p className="text-sm text-slate-600 mt-3">
              A guest whose nationality can&apos;t be resolved to a code (Thai nationals, or a code missing from both this table and the hardcoded fallback) is skipped from the TM30 export entirely rather than filed with a blank Nationality.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-6">
        ...
        {isAdding && (
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row gap-3 items-stretch md:items-end">
            ...
            <div className="flex-1 min-w-0">
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Eng</label>
              <input
                type="text"
                placeholder="e.g. THE FEDERAL REPUBLIC OF GERMANY"
            ...
            <div className="flex-1 min-w-0">
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Thai</label>
              <input
                type="text"
                placeholder="e.g. สหพันธสาธารณรัฐเยอรมันนี"
            ...
            <div className="w-full md:w-32 shrink-0">
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">TM30 Nationality</label>
              <input
                type="text"
                maxLength={3}
                placeholder="e.g. DEU"
                value={newForm.tm30_code}
                onChange={(e) => setNewForm({ ...newForm, tm30_code: e.target.value })}
                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#AAA024]/20"
              />
            </div>
            <div className="flex gap-2 shrink-0">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-2 py-2">
                    <input
                      key={`${row.id}-code-${row.tm30_code}`}
                      type="text"
                      maxLength={3}
                      defaultValue={row.tm30_code}
                      onBlur={(e) => handleFieldSave(row, "tm30_code", e.target.value)}
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-200">
            <div className="p-6">
              ...
              <p className="text-sm text-slate-500 mb-6">
                Are you sure you want to delete <span className="font-bold text-slate-700">{deletingRow.mews_nationality}</span>? TM30 exports will fall back to the hardcoded default code for this nationality, if one exists.
              </p>
