...
interface PropertySyncSettings {
  ...
  // Minute-level precision exists so this can mirror MEWS's own native
  // report window exactly (e.g. 12:15, not just whole hours).
  rr4_tm30_day_start_minute: number | null;
  // Wraps to the next day whenever its time-of-day isn't strictly after
  // start's (an overnight-window convention, like "22:00 to 06:00") - so
  // the window isn't forced to be exactly 24 hours; a property can
  // exclude a settling/audit gap (e.g. 14:00 to 12:00 next day, 22h).
  // PER REQUEST independently editable again after a stale value here once
                ...
                <tr key={prop.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5 text-center">
                    {prop.rr4_tm30_sync_enabled ? (
                      // The window's start (rr4_tm30_day_start_*, "Sweep Data
                      // From" in the Edit Schedule modal) - which calendar
                      // window the register is built over - not
                      // rr4_tm30_sync_hour/minute (when the job runs to
                      // build it). The two are unrelated: Chinatown's window
                      // starts 12:15 but the job that computes it can run at
                      // any hour after the day has closed.
                      <span className="bg-sky-50 text-sky-700 px-3 py-1.5 rounded-lg text-sm font-mono font-bold border border-sky-100">
                        {String(prop.rr4_tm30_day_start_hour ?? 0).padStart(2, '0')}:{String(prop.rr4_tm30_day_start_minute ?? 0).padStart(2, '0')}
                      </span>
                    ) : (
                      <span className="text-slate-300 text-sm">Off</span>
                    )}
              ...
              <div className="px-8 py-6 space-y-4 max-h-[70vh] overflow-y-auto">
                 ...
                 <div className="rounded-2xl border border-sky-500/20 bg-gradient-to-b from-sky-500/[0.07] to-transparent overflow-hidden">
                    ...
                    <div className="px-5 pb-5 space-y-4">
                       {/* Which hours+minutes of guest activity get swept
                           into one day's report. Minute-level precision
                           exists so this can mirror MEWS's own native
                           report window exactly - e.g. 12:15 to 12:15 the
                           next day, confirmed against a real MEWS export
                           for Chinatown. Both ends are independently
                           editable per request - "To" wraps to the next day
                           whenever its time-of-day isn't strictly after
                           "From"'s (an overnight-window convention, like
                           setting quiet hours "22:00 to 06:00"), so the
                           window doesn't have to be exactly 24h - e.g.
                           14:00 to 12:00 (next day) deliberately excludes a
                           2-hour settling/audit gap. CAUTION: a stale
                           non-default value here previously undercounted a
                           property's whole RR4 register by 76 guests
                           (confirmed against a real MEWS export - Chinatown
                           stuck at 14/12 showed 160 rows instead of the
                           correct 241) - double-check any non-zero end time
                           is actually intended before leaving this page.
                           Default 0:00/0:00 wraps to the full current-day
                           window, matching MEWS's own
                             ...
                             <div className="flex flex-col items-center">
                                <div className="flex items-center gap-1">
                                   <input
                                     ...
                                     min="0"
                                     max="23"
                                     className="w-11 bg-transparent text-center text-2xl font-mono font-bold text-white outline-none"
                                   ...
                                   <input
                                     ...
                                     max="59"
                                     className="w-11 bg-transparent text-center text-2xl font-mono font-bold text-white outline-none"
                                     value={editingProperty.rr4_tm30_day_start_minute ?? 0}
                                     onChange={(e) => setEditingProperty({...editingProperty, rr4_tm30_day_start_minute: parseInt(e.target.value) || 0})}
                                   />
                                </div>
                             ...
                             <div className="flex flex-col items-center">
                                <div className="flex items-center gap-1">
                                   ...
                                   <input
                                     ...
                                     max="59"
                                     className="w-11 bg-transparent text-center text-2xl font-mono font-bold text-white outline-none"
                                     value={editingProperty.rr4_tm30_day_end_minute ?? 0}
                                     onChange={(e) => setEditingProperty({...editingProperty, rr4_tm30_day_end_minute: parseInt(e.target.value) || 0})}
                                   />
                                </div>
                                <span className="text-[9px] font-bold text-white/25 tracking-widest">
                                  TO{((editingProperty.rr4_tm30_day_end_hour ?? 0) * 60 + (editingProperty.rr4_tm30_day_end_minute ?? 0))
                                      <= ((editingProperty.rr4_tm30_day_start_hour ?? 0) * 60 + (editingProperty.rr4_tm30_day_start_minute ?? 0))
                                      ? " (NEXT DAY)" : ""}
                                </span>
                             </div>
                             <span className="text-[10px] font-bold text-white/25 uppercase tracking-widest ml-1">Bangkok</span>
