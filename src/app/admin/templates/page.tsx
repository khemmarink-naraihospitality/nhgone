...
const RV_COMPARE_TOKENS: TokenDoc[] = [
  ...
  { name: "KnownDrift", description: "Lines whose only difference is already explained - the Google Sheet's \"×\" where our ASCII-only file (and MEWS's own real export) writes \"x\"" },
...
const TEMPLATE_CONFIG: Record<TemplateType, {
  ...
  rv_compare_email: {
    ...
    tokenNote: "Sent once a day (Time to Send below) comparing our RV Files revenue journal line by line against the RV Google Sheet (one tab per property), with every compared property's RV file attached. Amounts are compared as numbers, and a description that differs only by the sheet's \"×\" against our plain \"x\" (MEWS's own real export is plain ASCII too) is counted as known drift, not a difference. Nothing is sent on a day no property can be compared.",
    perProperty: false,
    hasSubject: true,
    previewable: true,
    hasScheduleFields: true,
    ...
    DetailTable: buildCompareSampleTable(
      ...
      [["Patong", "11403 · D · 1700.00<br><small>Card payment (Mastercard ****1740 Virtual, AGODA/#</small>",
       ...
       ["Chinatown", "5 lines<br><small>e.g. -1 × Room Revenue</small>", "Description",
        ...
        muted("Known drift - the Google Sheet holds \"×\" where our file (and MEWS's own real export) writes \"x\": get_rv_export folds every line to plain ASCII")]],
    ),
  }),
