          ...
          <div className="no-print fixed inset-0 z-50 overflow-auto" style={{ background: "#525659" }}>
            ...
            <div className="fixed top-24 bottom-4 right-4 z-10 bg-white text-black border border-black/10 shadow-2xl p-4 w-[340px] overflow-y-auto">
              ...
              {!regCardOccupation.trim() && (
                <div className="-mt-2 mb-3 text-[10px] text-black/40 italic">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {typeof effectiveRoomLocked(selectedReservation) === "boolean" && (
                            <button
                              ...
                              className={`relative inline-flex items-center shrink-0 w-9 h-5 rounded-full transition-colors ${effectiveRoomLocked(selectedReservation) ? "bg-indigo-500" : "bg-slate-300"}`}
                            >
                              <span className={`inline-flex items-center justify-center w-4 h-4 rounded-full bg-white shadow transform transition-transform ${effectiveRoomLocked(selectedReservation) ? "translate-x-[18px]" : "translate-x-0.5"}`}>
                                <svg className={`w-2.5 h-2.5 ${effectiveRoomLocked(selectedReservation) ? "text-indigo-600" : "text-slate-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                              </span>
                            </button>
                          )}
                        </div>
