...
`role_permissions.restricted_property` (nullable text) locks a role to one property: NULL/empty (the default, and Super Admin always regardless) means unrestricted — sees every property. Set to a `property_api_settings.property_name` value and every "Select Property" dropdown on RR3/Bills/BCP resolves through `src/lib/allowedProperties.ts`'s `getAllowedProperties()`, which returns just that single property instead of the full list — used for property-level staff (e.g. a "Lub d Bangkok Siam Front Office" role with only rr3/bills/bcp checked) who should never be able to pick another hotel out of the dropdown. Edited via the Role grid's "Property" column (a `<select>` per row, same immediate-save pattern as the boolean menu checkboxes). This is client-side gating only (matching the rest of the app's role_permissions security model, e.g. the admin route guard) — not enforced at the FastAPI layer, so it doesn't harden the backend against a user calling the API directly with a different `property_name`. Not yet wired into Data Mart/ST Files/Log Import/Templates — only the three pages a property-scoped role is expected to use.
