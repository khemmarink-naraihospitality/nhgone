...
- `/bcp` — Mews Business Continuity Plan: read-only snapshots (captured every 5 minutes) of a wide reservation window so front desk can keep working from the latest copy when MEWS is down. Backend `sync_service.get_bcp_snapshot` builds a MEWS-style Timeline: one `reservations/getAll` Extent call spanning today ±7 days (`_BCP_WINDOW_DAYS_BACK`/`_BCP_WINDOW_DAYS_FORWARD`) feeds room-row × date-column bars, joined against `orderItems/getAll` (rate-vs-product charge breakdown, gross/net, chunked 100 IDs at a time) and `serviceOrderNotes/getAll` (permission-gated — degrades to empty notes if the token lacks it), plus window-wide deduplicated lookups for reservation groups/rates/companies/business segments. Room rows come from `resources/getAll`, grouped/ordered by MEWS's own `resourceCategories.Ordering` field (not alphabetically — getting this wrong previously mis-ordered a live property's category groups), retried once on failure since a second failure silently collapses the grouping to empty; a resource with a `ParentResourceId` (e.g. a dorm room split into individually bookable beds) nests directly under its parent regardless of its own category. `Reservation.Origin`'s combined enum string (e.g. `CommanderInPerson`) is parsed back into MEWS's own "Reservation source" label, with a separate branch for channel-manager/OTA bookings whose Origin format differs entirely. Frontend `/bcp/page.tsx` has two tabs — **Timeline** (the grid, with date navigation and space search; clicking a reservation bar opens a detail panel with group/category/rate/company/travel-agency info, the expandable rate/items charge breakdown, notes, and a room-lock padlock indicator; clicking a room opens its own state/category panel) and **Payments** (today's `payments/getAll`) — plus a print-only plain table (`hidden print:block`, triggered by the Timeline tab's "Print Housekeeping Sheet" button, with a "Cleaned ✓" tick column) since the on-screen Timeline grid doesn't paginate for print. Router `bcp.py`: `GET /bcp/live` (build fresh, don't store — UI fallback when history is empty), `POST /bcp/capture` (build + store + prune to the newest `SNAPSHOTS_KEPT` (12) per property, i.e. 1 hour at 5-minute intervals — deliberately short since `bcp_snapshots` is whole-blob encrypted per row and was by far the largest table in the database at 48h/576 retention, driving a Supabase disk auto-expand), `GET /bcp/snapshots` + `GET /bcp/snapshot?id=` (history picker). `GET /bcp/auto-capture` is its own dedicated Vercel Cron entry (`*/5 * * * *`, separate from the hourly `/sync/auto` cron so BCP's frequency doesn't drag the main data sync along with it) or an APScheduler every-5-minutes job locally; each property is locked via the same `acquire_sync_lock`/`release_sync_lock` RPCs `daily_auto_sync` uses (4-minute timeout) so an overrunning capture can't overlap the next tick, and captures log to `sync_logs` **only on failure** (a success row per property per cycle would drown the Activity Log). Snapshots are stored in `bcp_snapshots` whole-blob Fernet-encrypted (`data.blob`, same pattern as `st_files_sync`)
...
`role_permissions.restricted_property` (nullable text) locks a role to one property: NULL/empty (the default, and Super Admin always regardless) means unrestricted — sees every property. Set to a `property_api_settings.property_name` value and every "Select Property" dropdown on RR3/Bills/BCP/ST Files resolves through `src/lib/allowedProperties.ts`'s `getAllowedProperties()`, which returns just that single property instead of the full list — used for property-level staff (e.g. a "Lub d Bangkok Siam Front Office" role with only rr3/bills/bcp/st_files checked) who should never be able to pick another hotel out of the dropdown. Edited via the Role grid's "Property" column (a `<select>` per row, same immediate-save pattern as the boolean menu checkboxes). This is client-side gating only (matching the rest of the app's role_permissions security model, e.g. the admin route guard) — not enforced at the FastAPI layer, so it doesn't harden the backend against a user calling the API directly with a different `property_name`. Not yet wired into Data Mart/Log Import/Templates.
...
| `bcp_snapshots` | 5-minute BCP front-desk snapshots (reservation Timeline ±7 days, today's payments, room housekeeping state), whole report Fernet-encrypted as `data.blob`; pruned to the newest 12 per property on every capture |
...
- **Backend-only tables** (all `bcp_*` override/log tables, `email_templates`, `bills_sync`, `smtp_settings`, `st_files_sync`, …) — RLS on, **no policies at all**. Nothing but the service role can touch them. The `rls_enabled_no_policy` INFO lint this produces is expected and correct, not a gap to close.
