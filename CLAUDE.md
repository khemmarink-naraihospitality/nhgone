...
- `/print-bill/[id]` — Fetches full itemized invoice data (`GET /bills/{id}/invoice`) and the property's HTML template (`GET /bills/template`), then does `<<Token>>` string substitution and renders on-screen via `dangerouslySetInnerHTML`. Token substitution (`renderInvoiceTemplate`), the `Invoice` type, and the `@page`-A4/page-break print CSS (`INVOICE_PRINT_CSS`) live in `src/lib/invoiceTemplate.ts` so both this page and the PDF export route below stay pixel-identical. `get_bill_invoice` (`sync_service.py`) checks `bills_sync` first and only falls back to live `bills/getAll`+`orderItems/getAll` if the bill isn't cached; `payments/getAll` is always called live since `payments` has no queryable Bill Id column. The page's "Download PDF" button does **not** use `window.print()` — it calls `GET /api/print-bill/pdf` (a Next.js Node runtime route, `src/app/api/print-bill/pdf/route.ts`) which launches headless Chromium server-side (`playwright-core`, using `@sparticuz/chromium`'s Linux binary when `process.env.VERCEL` is set, or the locally `npx playwright install`-ed Chromium in dev) and returns a ready-made multi-page A4 PDF for direct download — chosen over Python-side generation because WeasyPrint needs native Pango/GTK libs unavailable on Vercel's Python runtime, and `xhtml2pdf` (pure-Python, pip-only) cannot render Thai script (renders as blank/tofu glyphs) — both confirmed by hands-on testing before landing on headless Chromium, which reuses the exact same template/rendering path as the on-screen preview
